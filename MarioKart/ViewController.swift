...
class ViewController: UIViewController,
  ...
  @IBAction func didDoubleTapKart(_ sender: UITapGestureRecognizer) {
    ...
    // YOUR CODE HERE
      translate(kart: sender.view, by: view.frame.width){ //move kart forward by the width of the containing view
  ...
  private func moveKart(using gestureRecognizer: UIPanGestureRecognizer) {
    // YOUR CODE HERE
      let location = gestureRecognizer.location(in: view) //get location of gesture
      let kartView = gestureRecognizer.view // get reference to the kart
  ...
  private func resetKarts() {
    // YOUR CODE HERE
      UIView.animate(withDuration: 0.4) { //changes should occur within 0.4 seconds
          //reset the transformations for the karts
          self.kartView0.transform = .identity
          self.kartView1.transform = .identity
          self.kartView2.transform = .identity
          
          //reset the positions of karts
